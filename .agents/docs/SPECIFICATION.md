...
| Gemini CLI        | `mcpServers`      | JSON     | `~/.gemini/settings.json`                                                                                                      | `command`         | `env`         | `url`          |
| Continue          | `mcpServers`      | JSON     | `~/.continue/config.json`                                                                                                      | `command`         | `env`         | `url`          |
| Amazon Q          | `mcpServers`      | JSON     | `~/.aws/amazonq/mcp.json`                                                                                                      | `command`         | `env`         | `url`          |
| Trae              | `mcpServers`      | JSON     | `.trae/mcp.json` (project-level)                                                                                               | `command`         | `env`         | `url`          |
| BoltAI            | `mcpServers`      | JSON     | `~/Library/Application Support/BoltAI/mcp_config.json` (macOS)                                                                 | `command`         | `env`         | `url`          |
| LibreChat         | `mcpServers`      | **YAML** | `librechat.yaml` (project-level)                                                                                               | `command`         | `env`         | `url`          |
| Antigravity       | `mcpServers`      | JSON     | `~/.gemini/antigravity/mcp_config.json`                                                                                        | `command`         | `env`         | `url`          |
...
getmcp/
  ...
  packages/
    ...
    generators/                    # @getmcp/generators (v0.9.1)
      src/
    ...
    registry/                      # @getmcp/registry
      data/
        servers.json               # Aggregated server data (generated by sync pipeline)
        official-servers.json      # Curated list of official (first-party) server names
      src/
        index.ts                   # Registry API (search, filter, lookup)
        enrich.ts                  # GitHub enrichment: topic→category mapping, runtime inference
        enrichment-types.ts        # Zod schemas for enrichment and metrics
        extract-config.ts          # Extract canonical config from official format
        fetch-metrics.ts           # Metrics fetchers (GitHub, npm, PyPI, Docker)
        id-mapping.ts              # Slug generation from reverse-DNS names
        official-api-types.ts      # Zod schemas for official MCP registry API
        transform.ts               # Transform official entries to internal format
      scripts/
        sync.ts                    # Sync pipeline: fetches, enriches, writes data/servers.json
      tests/
        registry.test.ts
        enrich.test.ts
        id-mapping.test.ts
...
- Root key: **`mcp_servers`** (TOML table prefix, not `mcpServers`)
- Format: **TOML** (not JSON)
- **Renames**: `headers` -> `http_headers`
- **Converts**: `timeout` from milliseconds to seconds (`startup_timeout_sec`)
- Custom `serialize()` method outputs TOML via `smol-toml`
- Config path: `.codex/config.toml` (project), `~/.codex/config.toml` (global)
- Dual-scope app (supports both project and global config)
...
- **Passthrough**: canonical format IS the native format
- Root key: `mcpServers`
- Global-only config at `~/.gemini/settings.json`
...
- **Passthrough**: canonical format IS the native format
- Root key: `mcpServers`
- Global-only config at `~/.continue/config.json`
- Docs: https://docs.continue.dev/customize/model-providers/mcp

#### Amazon Q — `AmazonQGenerator`

- **Passthrough**: canonical format IS the native format
- Root key: `mcpServers`
- Global-only config at `~/.aws/amazonq/mcp.json`
...
- **Passthrough**: canonical format IS the native format
- Root key: `mcpServers`
- Project-only config at `.trae/mcp.json`
- Docs: https://docs.trae.ai/ide/model-context-protocol-mcp

#### BoltAI — `BoltAIGenerator`

- **Passthrough**: canonical format IS the native format
- Root key: `mcpServers`
- macOS-only; global config at `~/Library/Application Support/BoltAI/mcp_config.json`
- Docs: https://docs.boltai.com/docs/mcp/overview

#### LibreChat — `LibreChatGenerator`

- Root key: `mcpServers` (same as canonical)
- Format: **YAML** (not JSON)
- Custom `serialize()` method outputs YAML via `yaml`
- Project-only config at `librechat.yaml`
- Docs: https://www.librechat.ai/docs/configuration/mcp_servers
...
| Codex          | `mcp_servers`     | `command`         | `args`   | `env`         | `url`          | `http_headers`, `startup_timeout_sec`      | TOML   |
| Gemini CLI     | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | JSON   |
| Continue       | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | JSON   |
| Amazon Q       | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | JSON   |
| Trae           | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | JSON   |
| BoltAI         | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | JSON   |
| LibreChat      | `mcpServers`      | `command`         | `args`   | `env`         | `url`          | —                                          | YAML   |
...
```typescript
getServer(id: string): InternalRegistryEntry | undefined         // Lookup by official ID only
getServerBySlug(slug: string): InternalRegistryEntry | undefined // Lookup by slug directly
getServerOrThrow(id: string): InternalRegistryEntry              // Throws if not found
getServerIds(): string[]                                         // All IDs (reverse-DNS format)
getAllServers(): InternalRegistryEntry[]
searchServers(query: string): InternalRegistryEntry[]
getServersByCategory(category: string): InternalRegistryEntry[]
getCategories(): string[]
getServerCount(): number
findServerByCommand(command: string, args: string[]): InternalRegistryEntry | undefined
getServerMetrics(id: string): GetMCPMetricsType | undefined
getAllMetrics(): Map<string, GetMCPMetricsType>
getRawServerData(id: string): RegistryEntryType | undefined
getServersSortedBy(metric: string, limit?: number): InternalRegistryEntry[]
loadFromEntries(entries: InternalRegistryEntry[]): void
loadFromPath(path: string): void
resetRegistry(): void
```

**Search** matches against: id, slug, name, description, categories, author, and tags (case-insensitive).
...
The sync pipeline fetches entries, enriches them with GitHub metadata and metrics, generates URL-friendly slugs for display, and writes the aggregated data to `packages/registry/data/servers.json`.
...
interface LockInstallation {
  ...
  registry?: string; // Registry source name
}
```

Lock files with `version: 1` (slug-keyed) are automatically migrated to `version: 2` (official-name-keyed) on first read. In v2, the keys in `installations` are official reverse-DNS names (e.g. `io.github.github/github-mcp-server`).
...
readLockFile(filePath?: string, slugResolver?: SlugResolver): LockFile            // Read lock file (returns empty default if missing, migrates v1→v2)
...
The CLI caches registry data locally to avoid fetching on every invocation. Each configured registry gets its own subdirectory under the cache root.
...
**TTL:** 1 hour. Cache is considered fresh if `lastCheckedAt` is within the TTL window.

**Refresh modes (`--refresh` flag):**

| Mode        | Behaviour                                                                                     |
| ----------- | --------------------------------------------------------------------------------------------- |
| Incremental | Bypasses TTL, keeps existing cache, uses `updated_since` parameter for a delta fetch          |
| Full        | Clears all cache directories first, then fetches everything from scratch (no `updated_since`) |

When `--refresh` is used interactively, the CLI prompts the user to choose between incremental and full refresh. With `-y`, it defaults to incremental (fast, safe). Without `--refresh`, the cache is initialised normally with TTL-based freshness checks and automatic incremental fetches.
...
- **Workspace deps**: Internal deps use `*` version ranges for npm compatibility
