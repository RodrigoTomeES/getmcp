...
The sync pipeline fetches entries, enriches them with GitHub metadata and metrics, generates URL-friendly slugs, and writes the aggregated data to `packages/registry/data/servers.json`.
...
| `@getmcp/registry`   | 4          | 60      | Entry validation, lookup, search, categories, content integrity, enrichment, ID mapping, transform                                                                       |
| `@getmcp/cli`        | 18         | 297     | Path resolution, app detection/selection, config I/O, lock file, errors, format, preferences, utils, flags, add, check, doctor, find, import, list, remove, sync, update |
| **Total**            | **25**     | **515** |                                                                                                                                                                          |
