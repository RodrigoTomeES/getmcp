...
| `utils.ts`           | `parseFlags()` for CLI flag parsing (supports `--global`/`-g` and `--project` scope flags), `resolveAlias()` for command alias resolution, `shortenPath()` for display                                                                                                                          |
...
- **585 tests** across 24 test files
