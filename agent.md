...
| `lock.ts`            | Installation tracking via `./getmcp-lock.json`: `readLockFile()`, `writeLockFile()`, `trackInstallation()` (accepts optional `scope`), `trackRemoval()`, `getTrackedServers()`. `LockInstallation` has optional `scope` field                                            |
...
- **577 tests** across 24 test files
